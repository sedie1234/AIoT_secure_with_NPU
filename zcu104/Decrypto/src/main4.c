...
int main(int argc, char *argv[]) {
    ...
    if (argc != 4) {
        fprintf(stderr, "Usage: %s <enc_file.enc> <output file> <key_path>\n", argv[0]);
        return 1;
    }
    ...
    fread(&enc_file.file_size, sizeof(int), 1, enc_file_ptr);
    ...
    fwrite(decrypt_data, sizeof(uint8_t), enc_file.file_size, out_file);
    fclose(out_file);
    printf("Decrypted data saved to %s\n", out_file_path);
    printf("Decrypted data of length %d\n", enc_file.file_size);
