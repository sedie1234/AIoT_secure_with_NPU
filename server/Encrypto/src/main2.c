...
int main(int argc, char *argv[]) {
    ...
    if (argc != 4) {
        fprintf(stderr, "Usage: %s <data_length> <output file.enc> <key_path>\n", argv[0]);
        return 1;
    }

    char* arg_data_length = argv[1];
    char* out_file_path = argv[2];
    char* key_filename = argv[3];

    int data_length = atoi(arg_data_length);
    if (data_length <= 0) {
        fprintf(stderr, "Invalid data length: %s\n", arg_data_length);
        return 1;
    }
    ...
    for (int i = 0; i < data_length; i++) {
        data[i] = rand() % 256; // Random byte value
    }
